...
(function () {
  ...
  function setSyncMsg(m) {
    lastSyncMsg = m || '';
    const s = document.getElementById('rw-sync-status');
  ...
  function onSignedIn() {
    ...
    return sb.from('profiles').select('data').eq('id', cloudUser.id).maybeSingle().then(function (res) {
      if (res && res.error) { setSyncMsg('⚠️ Erro ao ler da nuvem: ' + res.error.message); }
      ...
      setSyncMsg('⚠️ Falha de rede ao ler da nuvem.');
      return cloudPush(localSnapshot());
    }).then(function () {
      refreshGGBar();
  ...
  function cloudPush(snap) {
    if (!sb || !cloudUser) return Promise.resolve({ ok: false });
    return sb.from('profiles').upsert({ id: cloudUser.id, data: snap || localSnapshot(), updated_at: new Date().toISOString() })
      .then(function (r) {
        if (r && r.error) { setSyncMsg('⚠️ Erro ao salvar na nuvem: ' + r.error.message); return { ok: false, error: r.error }; }
        setSyncMsg('Sincronizado ✓ · ' + new Date().toLocaleTimeString());
        return { ok: true };
      }, function (e) { setSyncMsg('⚠️ Falha de rede ao salvar.'); return { ok: false, error: e }; });
  }
  ...
  function cloudSyncNow() { if (cloudUser) onSignedIn(); }
  ...
  function updateAccountUI() {
    ...
    if (cloudUser) {
      box.innerHTML = '<p class="rw-acct-line">Conectado como <b>' + (cloudUser.email || 'você') + '</b></p>' +
        '<p id="rw-sync-status" class="rw-acct-sub">' + (lastSyncMsg || 'Seu progresso sincroniza automaticamente.') + '</p>' +
        '<button id="rw-sync" class="btn-ghost">Sincronizar agora</button>' +
        '<button id="rw-signout" class="btn-text">Sair</button>';
      const sy = document.getElementById('rw-sync'); if (sy) sy.addEventListener('click', cloudSyncNow);
